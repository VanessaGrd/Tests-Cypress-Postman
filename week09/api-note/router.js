...
router.put("/users/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const {name, email, password} = req.body;
     await db.query("update user set name = ?, email = ?, password = ? where id = ?", [name, email, password, id]);
    res.status(200).json({
      status: 200,
      message: "all users",
    });
...
router.put("/notes/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const {title, description, category, id_user} = req.body;
     await db.query("update note set title = ?, description = ?, category = ?, id_user = ? where id = ?", [title, description, category, id_user, id]);
    res.status(200).json({
      status: 200,
      message: "all users",
    });
